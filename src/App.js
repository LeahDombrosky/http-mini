...
class App extends Component {
  ...
  nameSearch() {
    ...
    axios
      .get(`${this.state.baseUrl}/api/buyers?searchLetters=${searchLetters}`)
      .then(response => {
        this.setState({ buyersToDisplay: response.data });
      })
      .catch(console.log);
  ...
  render() {
    const vehicles = this.state.vehiclesToDisplay.map(v => {
      return (
        <div key={v.id}>
          <p> Make: {v.make} </p> <p> Model: {v.model} </p>{" "}
          <p> Year: {v.year} </p> <p> Color: {v.color} </p>{" "}
          <p> Price: {v.price} </p>{" "}
    ...
    const buyers = this.state.buyersToDisplay.map(person => {
      return (
        <div key={person.id}>
          <p> Name: {person.name} </p> <p> Phone: {person.phone} </p>{" "}
